...
if(isset($_SESSION['admin_id']) and isset($_SESSION['admin_address_id']))
{
    ...
    $admin_id=$_SESSION['admin_id'];
            if(isset($_POST["submit"]))
            {
             ...
             if(!empty($data_street) and !empty($data_city) and !empty($data_state) and !empty($data_country) and !empty($data_pin) and !empty($admin_id))
                {
                 // echo $data_street;
      ...
      <div class="container">
          <div class="row">
            ...
            <div class="col-md-10" style="background-color:ed;margin-top:4%">
                ...
                ?>
                <form action="editAdmin.php?id=<?php echo $row['admin_id']; ?>&address_id=<?php echo $row_address['address_id']; ?>" method="post">
                    <div class="form-group">
                        <label for="fnameLabel">First Name</label>
                        <input type="text" class="form-control" id="fnameLabel"            value="<?php echo $row['admin_fname'];?>" required name="FnameData">
                    </div>
                    <div class="form-group">
                        <label for="LnameLabel">Last Name</label>
                        <input type="text" class="form-control" id="LnameLabel"  value="<?php echo $row['admin_lname'];?>" required name="LnameData">
                    </div>
                    <div class="form-group">
                        <label for="EmailLabel">Email</label>
                        <input type="email" class="form-control" id="emailLabel"  value="<?php echo $row['admin_email'];?>"  required name="EmailData">
                    </div>
                    <div class="form-group">
                        <label for="dobLabel">Date OF Birth</label>
                        <input type="date" class="form-control" id="DobLabel"  required value="<?php echo $row['admin_dob'];?>"  name="DobData">
                    </div>
                    <div class="form-group">
                        <label for="PhoneLabel">Phone Number</label>
                        <input type="number" maxlength="13" class="form-control" id="phoneLabel"  value="<?php echo $row['admin_phone'];?>"  required name="PhoneData">
                    </div> 
                    <div class="form-group">
